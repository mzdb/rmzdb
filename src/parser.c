...
int parserc_parse( struct parserc *self, char *xmlin ) {
    ...
    self->rootpos = xmlin; // point on xml first char
    ...
    if( self->last_state ) {
      ...
      self->err = 0;
      curnode = root = self->rootnode = new_nodec(); // allocate root node
    ...
    bang: // skip this unknwon tag type until '>' char
      let = *cpos;
      if( let == '>' ) {
        cpos++;
        goto val_1;
    ...
    name_gap:
      ...
      switch( *cpos ) {
        ...
        case ' ':
        case 0x0d: // CR carriage return char
        case 0x0a: // NL newline char
          cpos++;
          goto name_gap;
        case '>':
          cpos++;
    ...
    att_name:
      ...
      attname_len++;
#ifdef DEBUG
      printf("att_name: inc attname_len to %d\n", attname_len);
#endif
      cpos++;
      goto att_name;
    ...
    att_nameqs:
      ...
      attname_len++;
#ifdef DEBUG
      printf("att_nameqs: inc attname_len to %d\n", attname_len);
#endif
      cpos++;
      goto att_nameqs;
